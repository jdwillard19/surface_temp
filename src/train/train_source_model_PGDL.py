...
for hid_ct, n_hidden in enumerate(n_hidden_list):
    ...
    pretrain_dict = torch.load(pretrain_paths[hid_ct])['state_dict']
    ...
    for epoch in range(n_eps):
        ...
        if epoch % ep_per_save == 0 and epoch != 0:
            ...
            save_path = "../../models/"+lakename+"/PGDL_source_model_"+str(n_hidden)+"hid_"+str(epoch)+"ep"
    ...
    for ep_ct, eps in enumerate(ep_list):
        for targ_ct,target_id in enumerate(other_source_ids):
            ...
            for i, source_id in enumerate(top_ids): 
                ...
                #load source model
                load_path = "../../models/"+source_id+"/PGDL_source_model_"+str(n_hidden)+"hid_"+str(eps)+"ep"
