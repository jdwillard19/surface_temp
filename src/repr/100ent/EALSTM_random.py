...
n_rand = 32
site_ranges = np.array([site_ids[:100],site_ids[100:200],site_ids[200:300],
                        site_ids[-100:]])
...
for site_range in site_ranges:
    ...
    trn_path = "./100ent_"+str(site_ct)+"to"+str(len(site_range))+"_ctlstm_32rand_trn_data.npy"
    trn_date_path = "./100ent_"+str(site_ct)+"to"+str(len(site_range))+"_ctlstm_32rand_trn_dates.npy"

    #load train data
    if not os.path.exists(trn_path):
        (trn_data, trn_dates) = buildLakeDataForRNN_repr_trn(site_range,randomFeat=n_rand) 
    ...
    for r in range(n_runs):
        ...
        save_path = '../../../models/100ent_EALSTM_32rand_run'+str(r)
    ...
    for r in range(n_runs):
        # lstm_net = myLSTM_Net(n_total_feats, n_hidden, batch_size)
        # lstm_net = Model(input_size_dyn=n_features,input_size_stat=n_static_feats,hidden_size=n_hidden,no_static=True)
        load_path = '../../../models/100ent_EALSTM_32rand_run'+str(r)
        n_hidden = torch.load(load_path)['state_dict']['lstm.weight_hh'].shape[0]
        lstm_net = Model(input_size_dyn=n_features-n_static_feats,input_size_stat=n_static_feats,hidden_size=n_hidden,no_static=no_static)
        if use_gpu:
            lstm_net = lstm_net.cuda(0)
    ...
    for targ_ct, target_id in enumerate(site_range): #for each target lake
        ...
        rmse_per_lake[site_ct] = mat_rmse
...
final_output_df.to_csv("../../../results/100ent_EALSTM_32rand.csv")
