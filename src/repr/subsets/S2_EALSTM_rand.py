...
trn_path = "./s2_ctlstm_32rand_trn_data.npy"
trn_date_path = "./s2_ctlstm_32rand_trn_dates.npy"


#load train data
if not os.path.exists(trn_path):
    (trn_data, trn_dates) = buildLakeDataForRNN_repr_trn(site_ids,randomFeat=32) 
...
for r in range(n_runs):
    ...
    save_path = '../../../models/S2_EALSTM_32rand_run'+str(r)
...
for r in range(n_runs):
    # lstm_net = myLSTM_Net(n_total_feats, n_hidden, batch_size)
    # lstm_net = Model(input_size_dyn=n_features,input_size_stat=n_static_feats,hidden_size=n_hidden,no_static=True)
    load_path = '../../../models/S2_EALSTM_32rand_run'+str(r)
    n_hidden = torch.load(load_path)['state_dict']['lstm.weight_hh'].shape[0]
    lstm_net = Model(input_size_dyn=n_features-n_static_feats,input_size_stat=n_static_feats,hidden_size=n_hidden,no_static=False)
    if use_gpu:
        lstm_net = lstm_net.cuda(0)
...
for targ_ct, target_id in enumerate(site_ids): #for each target lake
    ...
    (tst_data, tst_dates, all_dates) = buildLakeDataForRNN_repr_tst([target_id],randomFeat=32) 
...
final_output_df.to_csv("../../../results/S2_EALSTM_32rand.csv")
