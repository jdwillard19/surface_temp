...
for site_ct, site_id in enumerate(train_ids):
    ...
    out_txt = out_f.read()
    match16 = re.findall(pattern="BEST_MODEL_PATH_16HID:../../models/"+site_id+"/glm_enulator_16hid_(\d+)ep\nMean\s+RMSE=(\d*\.?\d+)",string=out_txt,flags=re.M | re.DOTALL)
    err_per_glm16[site_ct] = match16[0][1]

    match32 = re.findall(pattern="BEST_MODEL_PATH_32HID:../../models/"+site_id+"/glm_enulator_32hid_(\d+)ep\nMean\s+RMSE=(\d*\.?\d+)",string=out_txt,flags=re.M | re.DOTALL)
    err_per_glm32[site_ct] = match32[0][1]

    match64 = re.findall(pattern="BEST_MODEL_PATH_64HID:../../models/"+site_id+"/glm_enulator_64hid_(\d+)ep\nMean\s+RMSE=(\d*\.?\d+)",string=out_txt,flags=re.M | re.DOTALL)
    err_per_glm64[site_ct] = match64[0][1]

    match128 = re.findall(pattern="BEST_MODEL_PATH_128HID:../../models/"+site_id+"/glm_enulator_128hid_(\d+)ep\nMean\s+RMSE=(\d*\.?\d+)",string=out_txt,flags=re.M | re.DOTALL)
