...
for site_ct, site_id in enumerate(train_ids):
    ...
    match32 = re.findall(pattern="BEST_MODEL_PATH_32HID:../../models/"+str(site_id)+"/LSTM_source_model_32hid_(\d+)ep\nRMSE=(\d*\.?\d+)",string=out_txt,flags=re.M | re.DOTALL)
    err_per_glm32[site_ct] = match32[0][1]

    match64 = re.findall(pattern="BEST_MODEL_PATH_64HID:../../models/"+str(site_id)+"/LSTM_source_model_64hid_(\d+)ep\nRMSE=(\d*\.?\d+)",string=out_txt,flags=re.M | re.DOTALL)
    err_per_glm64[site_ct] = match64[0][1]

    match128 = re.findall(pattern="BEST_MODEL_PATH_128HID:../../models/"+str(site_id)+"/LSTM_source_model_128hid_(\d+)ep\nRMSE=(\d*\.?\d+)",string=out_txt,flags=re.M | re.DOTALL)
