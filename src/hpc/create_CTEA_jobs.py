...
for name in rand_arr:
    ...
    script = "source /home/kumarv/willa099/takeme_repr.sh\n" #cd to directory with training script
    script2 = "python CTLSTM_random.py %s"%(l)
    # script3 = "python singleModel_customSparse.py %s"%(l)
    all= "\n".join([header,script,script2])
    sbatch = "\n".join(["sbatch job_CTrand%s.sh"%(l),sbatch])
    with open('./jobs/job_CTrand{}.sh'.format(l), 'w') as output:
        output.write(all)
...
for name in rand_arr:
    ...
    # script3 = "python singleModel_customSparse.py %s"%(l)
    all= "\n".join([header,script,script2])
    sbatch = "\n".join(["sbatch job_EArand%s.sh"%(l),sbatch])
    with open('./jobs/job_EArand{}.sh'.format(l), 'w') as output2:
        output2.write(all)
