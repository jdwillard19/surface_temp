...
for name in train_lakes:
    ...
    header = "#!/bin/bash -l\n#SBATCH --time=23:59:00\n#SBATCH --ntasks=8\n#SBATCH --mem=20g\n#SBATCH --mail-type=ALL\n#SBATCH --mail-user=willa099@umn.edu\n#SBATCH --output=train_PGDL_source_%s.out\n#SBATCH --error=train_PGDL_source_%s.err\n#SBATCH --gres=gpu:k40:2\n#SBATCH -p k40"%(l2,l2)
    script = "source /home/kumarv/willa099/takeme_train.sh\n" #cd to directory with training script
    script2 = "python train_source_model_PGDL.py %s"%(l)
    # script3 = "python singleModel_customSparse.py %s"%(l)
    all= "\n".join([header,script,script2])
    sbatch = "\n".join(["sbatch job_%s_PGDL_source.sh"%(l),sbatch])
    with open('./jobs/job_{}_PGDL_source.sh'.format(l), 'w') as output:
        output.write(all)


compile_job_path= './jobs/sbatch_script_PGDL_source.sh'
