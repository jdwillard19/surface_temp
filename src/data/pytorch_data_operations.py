...
def buildLakeDataForRNN_repr_tst(lakenames,seq_length=350,randomFeat=None,areaDepth=False,win_shift=175,
    ...
    for lake_ct, lakename in enumerate(lakenames):
        ...
        X_tst[:] = np.nan
        tst_dates = np.empty(shape=(n_tst_obs,seq_length),dtype='datetime64[ns]')
        # tst_dates[:] = np.nan
        for tst_ct,ind in enumerate(tst_obs_inds):
            if ind < seq_length-1:
...
def buildLakeDataForRNN_conus_NoLabel(lakename, data_dir, seq_length, n_features, \
                                            win_shift= 1, begin_loss_ind = 100, \
                                            outputFullTestMatrix=False, sparseCustom=None, \
                                            allTestSeq=False,n_static_feats=0,removeElevation=False,\
                                            postProcessSplits=True, randomSeed=0):
