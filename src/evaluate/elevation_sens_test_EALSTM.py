...
test_lakes = np.load("../../data/static/lists/elevation_test_ids.npy",allow_pickle=True)
...
for lake in test_lakes:
    feat_mat_raw = np.load("../../data/processed/"+lake+"/features_ea_conus.npy")
    if feat_mat_raw.shape[0] < 350:
        test_lakes = np.delete(test_lakes,np.where(test_lakes == lake))
        remove_ct += 1

print(remove_ct, "removed")
np.save("../../data/static/lists/elevation_test_ids_wRemoved.npy",test_lakes)
# sys.exit()
...
n_total_features = 10
feats = ['surface_area','lat','long','elevation','SW','LW','AT','RH','WSU','WSV']
#########################################################################################
...
for feat_ct, feat_ind_to_add_noise in enumerate(feat_inds):
    ...
    err_per_feat_ind[feat_ct] = np.median(rmse_per_lake)


print(err_per_feat_ind)
